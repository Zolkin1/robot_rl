...
class MoEPPO(PPO):
    """PPO with an auxiliary load-balancing loss for MoE actors.

    Only the update() method changes. Everything else — rollout
    collection, GAE computation, logging — is inherited as-is.
    """
    ...
    def update(self) -> dict[str, float]:
        ...
        ########################################
        ## CHANGE from RSL-RL 5.0.1 algorithm
        ...
        for batch in generator:
            ...
            ########################################
            ## CHANGE from RSL-RL 5.0.1 algorithm
            # We need the gate weights from the actor's MoE.
            # self.actor.mlp is our MixtureOfExperts module,
            # and self.actor.mlp.gate is the GatingNetwork.
            #
            # We re-run the gate on the current batch observations
            # to get differentiable gate weights.

            # Flatten obs the same way MLPModel does internally
            obs_flat = self.actor.get_latent(batch.observations)
            gate_weights = self.actor.mlp.gate(obs_flat)
            lb_loss = load_balancing_loss(gate_weights)
            ########################################

            ########################################
            ## CHANGE from RSL-RL 5.0.1 algorithm
            # Use the load balancing loss here too
            loss = (
                    surrogate_loss
                    + self.value_loss_coef * value_loss
                    - self.entropy_coef * entropy.mean()
                    + self.load_balance_coef * lb_loss
            )
            ########################################
