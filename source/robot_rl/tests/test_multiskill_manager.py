...
class TestComputeTransitionTime:
    """Tests for :meth:`MultiSkillManager.compute_transition_time`."""

    @pytest.fixture(scope="class")
    def msm(self) -> MultiSkillManager:
        """Library with ref-frame map built (both left/right ankles registered)."""
        m = MultiSkillManager(path=MERGED_LIBRARY_DIR, device=DEVICE)
        m.build_ref_frame_map(["right_ankle_roll_link", "left_ankle_roll_link"])
        return m

    def _first_half_periodic_indices(self, msm: MultiSkillManager) -> list[int]:
        """Return trajectory indices whose type is HALF_PERIODIC."""
        from robot_rl.tasks.manager_based.robot_rl.mdp.commands.traj_tracking.multiskill_manager import (
            _HALF_PERIODIC_INT,
        )
        return [
            i for i in range(msm.num_trajectories)
            if msm.data["traj_type"][i].item() == _HALF_PERIODIC_INT
        ]

    def test_identity_same_trajectory(self, msm):
        """Transitioning within the same trajectory should preserve old_t exactly."""
        hp = self._first_half_periodic_indices(msm)
        assert len(hp) >= 2
        # Spread t across each traj's period
        old_traj = torch.tensor([hp[0], hp[1], hp[-1]], dtype=torch.long)
        totals = msm.data["total_time"][old_traj]
        old_t = torch.tensor([0.1, 0.3, 0.75]) * totals

        new_t = msm.compute_transition_time(old_traj, old_t, old_traj)
        # For old_t < total, candidate_t = old_phi * total = old_t.
        assert torch.allclose(new_t, old_t, atol=1e-6)

    def test_same_convention_no_shift(self, msm):
        """Two trajs with matching ref-frame convention → no half-period shift."""
        hp = self._first_half_periodic_indices(msm)
        assert len(hp) >= 2
        # All test-library half-periodic trajs start on the same foot, so
        # candidate_frame == old_frame at candidate_t = old_phi * new_total.
        old_traj_idx = torch.tensor([hp[0], hp[0], hp[1]], dtype=torch.long)
        new_traj_idx = torch.tensor([hp[-1], hp[2], hp[-1]], dtype=torch.long)

        old_totals = msm.data["total_time"][old_traj_idx]
        new_totals = msm.data["total_time"][new_traj_idx]
        old_t = torch.tensor([0.1, 0.35, 0.6]) * old_totals
        old_phi = (old_t % old_totals) / old_totals

        new_t = msm.compute_transition_time(old_traj_idx, old_t, new_traj_idx)
        expected = old_phi * new_totals
        assert torch.allclose(new_t, expected, atol=1e-6)

    def test_frame_invariant_after_transition(self, msm):
        """After compute_transition_time, the ref frame at new_t matches old_t.

        This is the core correctness claim of the feature: whatever phi
        shift (or no shift) the helper applies, the resulting state should
        have the same reference frame in contact as the old state.
        """
        hp = self._first_half_periodic_indices(msm)
        old_traj_idx = torch.tensor([hp[0], hp[1], hp[-1], hp[2]], dtype=torch.long)
        new_traj_idx = torch.tensor([hp[-1], hp[2], hp[0], hp[-2]], dtype=torch.long)

        old_totals = msm.data["total_time"][old_traj_idx]
        old_t = torch.tensor([0.15, 0.4, 0.6, 0.9]) * old_totals
        new_t = msm.compute_transition_time(old_traj_idx, old_t, new_traj_idx)

        old_domain = msm._get_domain_indices(old_t, old_traj_idx)
        new_domain = msm._get_domain_indices(new_t, new_traj_idx)
        old_frame = msm._ref_frame_domain_map[old_traj_idx, old_domain]
        new_frame = msm._ref_frame_domain_map[new_traj_idx, new_domain]
        assert torch.equal(old_frame, new_frame)

    def test_shift_applied_when_frames_mismatch(self, msm):
        """Monkey-patch a flipped ref-frame map and verify the 0.5 shift fires."""
        hp = self._first_half_periodic_indices(msm)
        old_traj = hp[0]
        new_traj = hp[-1]
        old_total = msm.data["total_time"][old_traj].item()
        new_total = msm.data["total_time"][new_traj].item()

        # Save and swap new_traj's ref-frame map so its first-half convention
        # is opposite the old traj's. For a half-periodic traj with nd=1 and
        # expanded_domains=2, rows are [right, left]; flipping to [left, right]
        # forces candidate_frame != old_frame at any same-phi query.
        original = msm._ref_frame_domain_map.clone()
        try:
            ed = msm.data["expanded_domains"][new_traj].item()
            flipped = original[new_traj].clone()
            flipped[:ed] = torch.flip(flipped[:ed], dims=[0])
            msm._ref_frame_domain_map[new_traj] = flipped

            old_t = torch.tensor([0.2 * old_total])
            new_t = msm.compute_transition_time(
                torch.tensor([old_traj], dtype=torch.long),
                old_t,
                torch.tensor([new_traj], dtype=torch.long),
            )
            old_phi = (old_t % old_total) / old_total
            expected_phi = (old_phi + 0.5) % 1.0
            expected_t = expected_phi * new_total
            assert torch.allclose(new_t, expected_t, atol=1e-6)
        finally:
            msm._ref_frame_domain_map = original

    def test_non_half_periodic_no_shift(self, msm):
        """Transitions involving a non-half-periodic traj never get shifted."""
        from robot_rl.tasks.manager_based.robot_rl.mdp.commands.traj_tracking.multiskill_manager import (
            _HALF_PERIODIC_INT,
        )
        non_hp = [
        ...
        if not non_hp:
            pytest.skip("no non-half-periodic trajectories in the test library")

        hp = self._first_half_periodic_indices(msm)
        # Use hp → non_hp and non_hp → hp; both should skip the shift.
        old_traj_idx = torch.tensor([hp[0], non_hp[0]], dtype=torch.long)
        new_traj_idx = torch.tensor([non_hp[0], hp[0]], dtype=torch.long)
        old_totals = msm.data["total_time"][old_traj_idx]
        new_totals = msm.data["total_time"][new_traj_idx]
        old_t = torch.tensor([0.25, 0.25]) * old_totals
        old_phi = (old_t % old_totals) / old_totals
